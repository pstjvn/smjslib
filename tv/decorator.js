...
goog.scope(function() {
   ...
   *   itself to decorate.
   * @param  {boolean=} recursive If true, the children DOM elements will be
   ...
   */
  _.decorate = function(baseElement, recursive, opt_deco_cssclass) {
    ...
    if (decorator && element) {
      ...
      if (recursive) {
        _.decorateChildren(mainCompponent,
          goog.isString(opt_deco_cssclass) ? opt_deco_cssclass :
            _.decorateCssClass_);
      }
  ...
  _.decorateChildren = function(base, css_decorate_me) {
    ...
    for (var i = 0, len = collection.length; i < len; i++) {
      decorator = _.findDecorator_(goog.dom.classlist.get(
        collection[i]));
    ...
    var focused = goog.dom.getElementByClass(_.focusCssClass_,
      baseElement);
